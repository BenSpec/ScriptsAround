...
EpisExact<-memoise(function(k)
{if (k==9) return(function(n){n==0})
 if (k==10) return(function(n){n==1})
 if (k==11) return(function(n){n==2})
 if (k==12) return(function(n){n==3})
 if (k==13) return(function(n){n==5})	
if (k==14) return(function(n){n==6})
if (k==15) return(function(n){n==7})
if (k==16) return(function(n){n==8})})
...
# + I know add:==0, ==1, ... up to ==9, hence 17 epistemic states
#Order = (=4, u35,u26,u17,u08,p35,p26,p17,p18,=0,=1,=2,=3,=5,=6,=7,=8)
#So there are exactly 9 epistemic states, nepis<-17
...
{if (x<5) return(EpisUniform(x))
 if(x>=5 & x<9) return(EpisPeaked(x-4))
 else return(EpisExact(x))
	})							
...
#Weights
ProbaEpisAux<-c(c(0.1,4,16,64,256,1,4,16,64),4*c(11,8,4,2,2,4,8,11))
...
#associating messages with numbers: 0 is 'between 4-0 and 4+0', ....4 is 'between 0 and 8', 5 is 'around 4', 6= exactly 0, 7 = exactly 1, 8 = exctly 2, 9 = exactly 3, 10=exactly 5, 11 = exactly 6, 12 = exactlt 7, 13 = exactly 8, So there are 13 messages, nmessages=13
...
L0exact<-function(n,u)
{if (u>=6 & u<=9) return((u-6)==n)
 else return((u-5)==n)	
	}
...
{if (u==5) return(Laround(n))
 if (u<5) return(Lbetween(u)(n))
 if (u>5) return(L0exact(n,u))
	})


L0table<- outer(0:(nworlds-1), 0:(nmessages-1), Vectorize(L0))
colnames(L0table)<-c("Four", "b_3_5", "b_2_6", "b_1_7","b_0_8", "around", "Zero", "One","Two","Three","Five","Six","Seven","Eight")
...
#Uk(u,epis) = - KL divergence between L(k-1)_u and epis, where L(k-1)_u is the posterior distribution of level(k-1)-speaker after processsing message u, and epis is the epistemic state of the speaker, - cost(u) [not indicated here because all costs = 0]
...
{table<-outer(0:(nworlds-1), 0:(nmessages-1), Vectorize(function(n,u){L(k,n,u)}))
 	colnames(table)<-c("Four", "b_3_5", "b_2_6", "b_1_7","b_0_8", "around", "Zero", "One","Two","Three","Five","Six","Seven","Eight")
...
colnames(table)<-c("Four", "b_3_5", "b_2_6", "b_1_7","b_0_8", "around", "Zero", "One","Two","Three","Five","Six","Seven","Eight")
row.names(table)<-c("=4","u_3_5", "u_2_6", "u_1_7", "u_0_8","p_3_5", "p_2_6", "p_1_7", "p_0_8","=0","=1","=2","=3","=5","=6","=7","=8")
return(table)}
